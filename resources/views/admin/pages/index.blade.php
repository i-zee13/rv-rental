@extends('admin.layout')

@section('title','Pages')

@section('content')
<div class="admin-page-header">
    <h1 class="text-xl font-semibold">Pages</h1>
    <a href="{{ route('admin.pages.create') }}" class="bg-yellow-500 text-black px-3 py-2 rounded">Add Page</a>
</div>

@if(session('success'))<div class="text-green-600 mb-3">{{ session('success') }}</div>@endif

<div class="admin-table-wrap bg-white border rounded-lg p-3">
<table class="admin-datatable w-full display">
    <thead>
        <tr>
            <th class="p-3 text-left">#</th>
            <th class="p-3 text-left">Slug</th>
            <th class="p-3 text-left">Title (EN)</th>
            <th class="p-3 text-left">Published</th>
            <th class="p-3 text-left no-sort">Actions</th>
        </tr>
    </thead>
    <tbody>
        @foreach($pages as $p)
            @php $t = $p->translations->firstWhere('locale','en') ?? $p->translations->first(); @endphp
            <tr class="border-t">
                <td class="p-3">{{ $p->id }}</td>
                <td class="p-3">{{ $p->slug }}</td>
                <td class="p-3">{{ $t->title ?? '-' }}</td>
                <td class="p-3">{{ $p->is_published ? 'Yes' : 'No' }}</td>
                <td class="p-3">
                    <x-admin-table-actions
                        :view="$p->is_published ? route('pages.show', $p->slug) : null"
                        :edit="route('admin.pages.edit', $p->id)"
                        :delete-action="route('admin.pages.destroy', $p->id)"
                        delete-confirm="Delete this page?"
                    />
                </td>
            </tr>
        @endforeach
    </tbody>
</table>
</div>
@endsection
