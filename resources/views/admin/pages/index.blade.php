...
<div class="admin-page-header">
    ...
    <a href="{{ route('admin.pages.create') }}" class="bg-yellow-500 text-black px-3 py-2 rounded">Add Page</a>
</div>
    ...
    <tbody>
        @foreach($pages as $p)
            ...
            <tr class="border-t">
                ...
                <td class="p-3">{{ $p->is_published ? 'Yes' : 'No' }}</td>
                <td class="p-3">
                    <a href="{{ route('admin.pages.edit', $p->id) }}" class="text-blue-600 mr-2">Edit</a>
                    <form action="{{ route('admin.pages.destroy', $p->id) }}" method="POST" style="display:inline">@csrf @method('DELETE')<button class="text-red-600">Delete</button></form>
                </td>
            </tr>
        @endforeach
    </tbody>
